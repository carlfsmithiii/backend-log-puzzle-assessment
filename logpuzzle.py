...
def download_images(img_urls, dest_dir):
    ...
    for img_url in img_urls:
        ...
        with open(filename, 'rb') as f_obj:
            with open(os.path.join(dest_dir, imagename + '.jpg'), 'wb') \
                    as target:
                target.write(f_obj.read())
            img_num += 1
    with open(os.path.join(dest_dir, 'index.html'), 'wt') as target:
        target.write("<html>\n<style>img{margin:0}</style>\n<body>\n")
        for filename in sorted(os.listdir(dest_dir), key=str.lower):
